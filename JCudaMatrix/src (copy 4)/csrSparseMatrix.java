...
public class csrSparseMatrix {
	...
	private int m; // rows, columns; IMPORTANT: we support square matrices only so m,n used very sloppily
	...
	public csrSparseMatrix(cusparseHandle handle, float[][] e, int m, int n){
		...
		cudaMemcpy(AcooRowIndex_gpuPtr, Pointer.to(AcooRowIndex_host), nnz*Sizeof.INT,          cudaMemcpyHostToDevice);
		cudaMemcpy(AcooColIndex_gpuPtr, Pointer.to(AcooColIndex_host), nnz*Sizeof.INT,          cudaMemcpyHostToDevice);
		cudaMemcpy(AcooVal_gpuPtr,      Pointer.to(AcooVal_host),      nnz*Sizeof.FLOAT,        cudaMemcpyHostToDevice);
		...
		cusparseXcoo2csr(handle, AcooRowIndex_gpuPtr, nnz, m, AcsrRowIndex_gpuPtr, CUSPARSE_INDEX_BASE_ZERO);
	...
	public float [] LuSolve(denseVector b_gpuPtr, boolean iLuBiCGStabSolve){
		...
		// - (AcsrRowIndex_gpuPtr, AcooColIndex_gpuPtr, AcooVal_gpuPtr) is CSR of A on device memory,
		...
		// Nvidia's original example overwrites matrix A, which is not ideal when later using iLuBiCGStabSolve
		...
		cudaMemcpy(iLUcsrRowIndex_gpuPtr, AcsrRowIndex_gpuPtr, (m+1)*Sizeof.INT,          cudaMemcpyDeviceToDevice);
		cudaMemcpy(iLUcooColIndex_gpuPtr, AcooColIndex_gpuPtr, nnz*Sizeof.INT,          cudaMemcpyDeviceToDevice);
		...
		cusparseScsrilu02_bufferSize(handle, m, nnz,
				descr_iLU, AcooVal_gpuPtr, AcsrRowIndex_gpuPtr, AcooColIndex_gpuPtr, info_iLU, pBufferSize_iLU);
		cusparseScsrsv2_bufferSize(handle, trans_L, m, nnz, 
				descr_L, AcooVal_gpuPtr, AcsrRowIndex_gpuPtr, AcooColIndex_gpuPtr, info_L, pBufferSize_L);
		cusparseScsrsv2_bufferSize(handle, trans_U, m, nnz, 
				descr_U, AcooVal_gpuPtr, AcsrRowIndex_gpuPtr, AcooColIndex_gpuPtr, info_U, pBufferSize_U);
		...
		cusparseScsrilu02_analysis(handle, m, nnz, descr_iLU,
				AcooVal_gpuPtr, AcsrRowIndex_gpuPtr, AcooColIndex_gpuPtr, info_iLU, 
				policy_iLU, pBuffer);
		...
		// we need to switch to DEVICE before using cusparseXcsrilu02_zeroPivot, for obscure reasons, and switch back to HOST afterwards
		cusparseSetPointerMode(handle,CUSPARSE_POINTER_MODE_DEVICE);
		if (CUSPARSE_STATUS_ZERO_PIVOT == cusparseXcsrilu02_zeroPivot(handle, info_iLU, structural_zero)){
			int [] sz =new int[1];
		...
		cusparseSetPointerMode(handle,CUSPARSE_POINTER_MODE_DEVICE);		
		if (CUSPARSE_STATUS_ZERO_PIVOT == cusparseXcsrilu02_zeroPivot(handle, info_iLU, numerical_zero)){
			int [] nz =new int[1];
		...
		// step 6: solve L*z = x
		cusparseScsrsv2_solve(handle, trans_L, m, nnz, Pointer.to(one_host), descr_L,
				iLUcooVal_gpuPtr, iLUcsrRowIndex_gpuPtr, iLUcooColIndex_gpuPtr, info_L,
				b_gpuPtr.getPtr(), z_gpuPtr, policy_L, pBuffer);

		// step 7: solve U*y = z
		cusparseScsrsv2_solve(handle, trans_U, m, nnz, Pointer.to(one_host), descr_U,
				iLUcooVal_gpuPtr, iLUcsrRowIndex_gpuPtr, iLUcooColIndex_gpuPtr, info_U,
				z_gpuPtr, x_gpuPtr, policy_U, pBuffer);
		...
		if (iLuBiCGStabSolve){
			...
			// see paper by Nvidia using cublas1 http://docs.nvidia.com/cuda/incomplete-lu-cholesky/index.html
			// this was also useful: 			//https://www.cfd-online.com/Wiki/Sample_code_for_BiCGSTAB_-_Fortran_90
			...
			//we create a number of pointers according to the method, and subsequently allocate memory
			...
			float[] float_host=new float[1]; //used as helper variable to pass floats


			// BiCGStab numerical parameters
			int maxit = 1000; // maximum number of iterations
			float tol=1e-3f; // tolerance nrmr / nrmr0[0], which is size of current errors divided by initial error
			...
			// 1 : compute initialresidual r = b − A x0 ( using initial guess in x )
			cusparseScsrmv ( handle , CUSPARSE_OPERATION_NON_TRANSPOSE , n , n , nnz,Pointer.to(one_host),descrA , AcooVal_gpuPtr, AcsrRowIndex_gpuPtr, AcooColIndex_gpuPtr  , x_gpuPtr , Pointer.to(zero_host) , r ) ;
			cublasSscal (cublashandle, n ,  Pointer.to(minus_one_host), r , 1 ) ;
			cublasSaxpy (cublashandle, n , Pointer.to(one_host) , b_gpuPtr.getPtr(), 1 , r , 1 ) ;
			...
			// 3 : repeat until convergence (based on maximum number of iterations and relative residual)
			...
			for ( int i =0; i<maxit ; i++){
				...
				if ( i > 0) { 
					// 1 2 : \beta = (\rho{ i } / \rho { i − 1}) ( \alpha / \omega )
				...
				// 1 5 : A \ hat{p} = p ( sparse lower and upper triangular solves )
				cusparseScsrsv_solve( handle , CUSPARSE_OPERATION_NON_TRANSPOSE ,
						n , Pointer.to(one_host) , descr_L ,iLUcooVal_gpuPtr, iLUcsrRowIndex_gpuPtr, iLUcooColIndex_gpuPtr,
						infoL , p , t ) ;

				cusparseScsrsv_solve ( handle , CUSPARSE_OPERATION_NON_TRANSPOSE ,
						n , Pointer.to(one_host) , descr_U , iLUcooVal_gpuPtr, iLUcsrRowIndex_gpuPtr, iLUcooColIndex_gpuPtr,
						infoU , t , ph ) ;
				...
				cusparseScsrmv ( handle , CUSPARSE_OPERATION_NON_TRANSPOSE , n , n ,nnz, Pointer.to(one_host) ,
						descrA , AcooVal_gpuPtr, AcsrRowIndex_gpuPtr, AcooColIndex_gpuPtr, ph , Pointer.to(zero_host) , q ) ;
				...
				// 2 3 : M \ hat{ s } = r ( sparse lower and upper triangular solves )
				...
				cusparseScsrmv ( handle , CUSPARSE_OPERATION_NON_TRANSPOSE , n , n ,nnz, Pointer.to(one_host) ,
						descrA , AcooVal_gpuPtr, AcsrRowIndex_gpuPtr, AcooColIndex_gpuPtr, s , Pointer.to(zero_host) , t ) ;
				...
				System.out.println("nrmr: "+nrmr[0] +" nrmr0: "+nrmr0[0]+ " alpha: "+alpha[0]+ " beta: "+beta[0]+ " rho: "+rho[0]+ " temp: "+temp[0]+ " temp2: "+temp2[0]+ " omega: "+omega[0]);
		...
		cudaMemcpy(Pointer.to(result_host), x_gpuPtr, m*Sizeof.FLOAT, cudaMemcpyDeviceToHost); //copy results back
	...
	public float[] mldivide(denseVector  b_gpuPtr){
		...
		cudaMalloc(result_gpuPtr, n*Sizeof.FLOAT); //allocate gpu memory for results
		...
		cusparseStatus = cusparseScsrsm_analysis(handle, CUSPARSE_OPERATION_NON_TRANSPOSE, m, nnz, descrA, AcooVal_gpuPtr, AcsrRowIndex_gpuPtr, AcooColIndex_gpuPtr, info);

		if(cusparseStatus != CUSPARSE_STATUS_SUCCESS)
			System.out.println("in csrSparseMatrix.mldivide(), problem with cusparseScsrsm_analysis, cusparseStatus = "+ cusparseStatus);

		cusparseStatus = cusparseScsrsm_solve( handle, CUSPARSE_OPERATION_NON_TRANSPOSE, m, 1, Pointer.to(one_host), descrA, AcooVal_gpuPtr, AcsrRowIndex_gpuPtr, AcooColIndex_gpuPtr, info, b_gpuPtr.getPtr(), n, result_gpuPtr, n);

		if(cusparseStatus != CUSPARSE_STATUS_SUCCESS)
			System.out.println("in csrSparseMatrix.mldivide(), problem with cusparseScsrsm_solve, cusparseStatus = "+ cusparseStatus);

		// copy results back to host
		cudaMemcpy(Pointer.to(result_host), result_gpuPtr, n*Sizeof.FLOAT, cudaMemcpyDeviceToHost);
	...
	public void print_coo(){
		...
		for (int i=0; i<nnz; i++)
		{
			System.out.printf("cooRowInded_host[%d]=%d  ",i,AcooRowIndex_host[i]);
			System.out.printf("cooColInded_host[%d]=%d  ",i,AcooColIndex_host[i]);
