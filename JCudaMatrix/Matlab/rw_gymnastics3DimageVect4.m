...
sigma_grad=0.3; %standard deviation for the intensity gradient weight
sigma_mean=0.3; %standard deviation for the difference from mean weight
mean_source = 0.4;
...
size(L)
size(seeds)
length(seeds)+length(L)


"TTT1"
size(Sink)
[Uttt, Ittt] = unique(seeds, 'first');
x = seeds;
x(Ittt) = [];
x
size(seeds)
